...
description: >
  INTERNAL — all gate agents (implementation, security, tester) and the Loop.
...
findings:
  - severity: critical | major | minor
    gate: implementation | security | tester
    owner: build | test | security      # routes the retry
...
| No `owner` | default by gate: implementation→build, tester→test, security→security |
...
Applies identically to Implementation, Security and Tester. Nothing else is a pass condition.
...
| security severity | Security | High/Critical is a scanner classification, not a judgement |

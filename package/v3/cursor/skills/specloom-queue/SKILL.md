...
name: specloom-queue
description: >
  INTERNAL — Project Manager and Loop. Brief dependency order, tracker-independent stage mapping,
...
| **@specloom** | decides whether to Task the Loop with a `run_set` |
| **specloom-loop** | reads the queue to confirm a Brief, never to select new work |

PM must **never** Task the Loop. That rule lives in **specloom-contract** and this skill does not override it.

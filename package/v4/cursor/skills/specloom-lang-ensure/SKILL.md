...
description: >
  INTERNAL — specloom-project-manager (init). After languages locked: ensure code-{lang} + test-{lang}
...
| Agent | Loads |
|-------|--------|
| specloom-implementation / backend / database | `specloom-coding` + each `code-{L}` for Task languages |
| specloom-tester / backend / database | `specloom-testing` + each `test-{L}` |

No new per-language peer agents.
...
A generated `code-*` / `test-*` is **committed to the product repo** at
`.specloom/skills/<name>/SKILL.md`, not regenerated per run.

```
1. generate from the template
2. write to .specloom/skills/<name>/SKILL.md
3. commit on the current work branch: chore(<brief>): pin <name> standards
4. record name + template version in the Overview
```

| Situation | Action |
|-----------|--------|
| Absent | generate, commit, mount |
| Present | **mount as-is** — never regenerate |
| Present, older template version | mount as-is; note as `minor` |
| User asks to refresh | regenerate, show the diff, commit on approval |

In v2 these were generated on demand and never stored, so two Briefs in one project could build
under different rules for the same language, with nothing recording that it happened. Pinning
turns the standards into a reviewable artifact with a diff.

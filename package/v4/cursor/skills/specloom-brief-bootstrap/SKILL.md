...
description: >
  INTERNAL — specloom-project-manager. Coverage map; commit Overview/GitHub; Phases/Briefs; advisory.
  Not user-invokable.
...
Must be **solid** (automation-ready detail) before Ready Briefs for that scope — loop dialogue until the **specloom-init-dialogue** checklist is complete or its gaps are explicitly accepted:
...
- [ ] Queue head Ready → main thread may launch the `specloom-run-set` workflow (unless `manual`)
...
- Replacing specloom-project-manager peer (init bootstraps; brief maintains)  
- Fat v1 docs tree inside app repo

---

## Choosing the tracker — once, here

Before any Phase or Brief exists, ask:

```
Which tracker for this product?
  linear — richer hierarchy and relations. Free plan caps at 250 issues / 2 teams.
  github — no issue cap, lives beside the code. Needs gh CLI 2.94+.
```

Record it on the Overview and never ask again:

```yaml
tracker: github
tracker_ref: acme/budget-tracker
```

**This is the only point at which the tracker is chosen.** It is not re-evaluated when one fills
up, and never mid-product — see the one-tracker invariant in **specloom-tracker**. Changing it
later is a deliberate migration a human runs, not an agent decision.

If the user has no preference: **github** when the product already has a repo and no tracker
exists yet; **linear** when they already run Linear for other work. Say which you picked and why.

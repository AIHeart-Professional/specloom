...
description: >
  INTERNAL — main thread only. Reads the product goal and everything built so far, researches
...
No valid `DISCOVERY_HANDOFF` from the main thread → reply JSON only:

```json
{"type":"ACCESS_DENIED","from":"specloom-discovery","reason":"main_thread_only"}
```
...
The main thread then Tasks **specloom-document** to write the proposal document, and reports to
the user. Accepted proposals go to PM as Brief input — **on the user's word, never on yours**.

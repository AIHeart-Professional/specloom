...
- Follow the **Rules of React** ó violations are bugs, not style preferences.
...
- Side effects belong in **event handlers** or **`useEffect`** ó never in render.

## Rules of React ó Purity
...
- **Derived data** ? compute during render from props/state ó do not duplicate in state
- **Updates** ? use state setters (`setState`, functional updates) ó never mutate state directly
...
- Extract repeated stateful logic into **custom Hooks** ó do not copy-paste effect blocks.

## Component Design

- **One component, one responsibility** ó split when render logic hides behavior or exceeds ~150 lines.
...
- **Lift state** only as high as needed ó avoid global state for local UI concerns.
- Prefer **key** prop to reset component state over effect-driven resets.
- For complex state, use **`useReducer`** with typed actions ó avoid many coupled `useState` calls.
- URL, server cache, and form libraries (React Query, Router, React Hook Form) own their domains ó do not mirror in redundant local state.
...
- Always declare **complete dependency arrays** ó do not lie to the linter.
...
- Do not premature-optimize with `memo` / `useMemo` / `useCallback` ó measure first.
- Use `React.memo` for expensive pure components receiving stable props.
- Use **`key`** on lists ó stable, unique IDs, never array index when list can reorder.
...
- Loading, empty, and error states are required ó never blank screens on failure.
...
- Prop drilling more than 2ñ3 levels without context or composition refactor.

## Review Checklist

- [ ] Render is pure ó no side effects or mutations

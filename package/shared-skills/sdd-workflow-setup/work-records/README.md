...
├── testing.md          # After tests pass — sdd-validation output
...
```
First task complete
  → sdd-updates creates manifest.json + work-done.md

Each task complete
  → sdd-updates updates manifest.json + appends work-done.md + spec Changes

All tasks complete
  → manifest.status = awaiting_tests

Work validation
  → sdd-validation reads manifest.json only
...
Tests pass
  → sdd-updates writes implementation.md, testing.md, completion.json

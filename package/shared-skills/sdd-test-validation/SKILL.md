---
name: sdd-test-validation
description: >-
  INTERNAL — sdd-validation agent only. Test execution + coverage/quality scores
...
Invoked by **sdd-validation** when `validation_type: test`.
...
|------|-------|
| 1 | **sdd-test-unit** |
| 2 | **sdd-test-integration** |
| 3 | **sdd-test-e2e** |
| 4 | **sdd-test-coverage** |
| 5 | **sdd-test-regression** |
...
| backend | **test-python**, **test-typescript** (if TS in manifest) |
| database | **test-postgres** |

Also read **sdd-work-records** skill.
...
**sdd-orchestrator** routes fixes to domain agents. Max **3** (`testingAttempts`). Attempt 3 fail → spec `incomplete`, `blocked_work.json`.
...
**sdd-updates** writes `testing.md` from `test_run` on pass.

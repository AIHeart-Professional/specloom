...
+-- testing.md          # After tests pass — specloom-validator output
...
```
First task complete
  ? specloom-update-knowledgebase creates manifest.json + work-done.md

Each task complete
  ? specloom-update-knowledgebase updates manifest.json + appends work-done.md + spec Changes

All tasks complete
  ? manifest.status = awaiting_tests

Work validation
  ? specloom-validator reads manifest.json only

Test gate
  ? specloom-validator reads manifest.json + spec Changes + Requirements + codebase
  ? loads test-* skills per manifest.layers

Tests pass
  ? specloom-update-knowledgebase writes implementation.md, testing.md, completion.json

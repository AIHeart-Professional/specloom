---
name: records-keeper-work-records
description: >-
  INTERNAL — sdd-records-keeper (write), sdd-qa-tester and sdd-qa-tester (read manifest only).
...
|------|----------|--------------|
| **manifest.json** | Machines | **Read** — sdd-qa-tester, sdd-qa-tester (work) |
...
```
task complete → sdd-records-keeper updates manifest.json + work-done.md + spec Changes
all tasks done → sdd-qa-tester(work) reads manifest.json
              → sdd-qa-tester reads manifest.json + spec Changes + Requirements + codebase
tests pass   → sdd-records-keeper writes implementation.md, testing.md, completion.json
...
| `tests_passed` | sdd-qa-tester passed; finalize records |
| `archived` | Spec archived |

### Update rules (sdd-records-keeper)
...
Orchestrator always passes this when delegating sdd-qa-tester.
...
Orchestrator passes when delegating sdd-qa-tester `work`.

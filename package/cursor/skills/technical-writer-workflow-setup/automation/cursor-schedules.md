...
**Agent (required):** Select **`sdd-project-lead`** in each automation's agent settings — not `sdd-workflow-coordinator` or any other sub-agent.

Workflow procedures live in **global skills** (`workflow-coordinator-*`), not repo markdown.  
**sdd-project-lead** Task-delegates **sdd-workflow-coordinator** and other sub-agents. Sub-agents reply in JSON **to the project lead only**. The **final automation message to you must be natural language** — never raw JSON.
...
**Agent:** `sdd-project-lead`

**Prompt:**

```
You ARE sdd-project-lead (sole user-facing entry). Follow ~/.cursor/agents/sdd-project-lead.md.
...
```
You ARE sdd-project-lead. Notify-only run — do not implement, validate, or git push.
...
1. **Agent:** `sdd-project-lead` on every automation
...
- Agent was **sdd-workflow-coordinator** (or default) instead of **sdd-project-lead**
...
Fix: **sdd-project-lead** agent + prompts above.
